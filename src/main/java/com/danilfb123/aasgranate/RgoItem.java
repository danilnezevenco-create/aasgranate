...
public class RgoItem extends Item implements GeoItem {
    ...
    // Чем дольше игрок держит ПКМ (до MAX_CHARGE_TICKS), тем дальше летит граната.
    // На 2 секундах (40 тиков) и дальше — полная сила.
    private static final int   MAX_CHARGE_TICKS    = 40;   // 2 секунды = 40 тиков
    private static final float MAX_THROW_VELOCITY  = 2F;    // максимальная сила броска
    private static final float MIN_THROW_VELOCITY  = 0.75F; // минимальная сила броска

    public RgoItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        level.playSound(null, player.getX(), player.getY(), player.getZ(),
                ModSounds.RGO_PIN_PULL.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity entityLiving, int timeLeft) {
        ...
        level.playSound(null, player.getX(), player.getY(), player.getZ(),
                ModSounds.RGO_THROW.get(), SoundSource.PLAYERS, 1.0F, 1.0F);

        if (!level.isClientSide) {
            RgoEntity projectile = new RgoEntity(level, player);
        ...
        consumer.accept(new IClientItemExtensions() {
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (this.renderer == null) {
                    this.renderer = new com.danilfb123.aasgranate.client.RgoItemRenderer();
                }
    ...
    private PlayState predicate(AnimationState<RgoItem> event) {
